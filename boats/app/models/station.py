...
class Station(db.Model, Base):
    ...
    drones = db.relationship('Drone', backref='station', lazy=True)

    def __repr__(self):
        return "<Station {} (long:{}, lat:{})>".format(self.id, self.longitude, self.latitude)

    @classmethod
    def create(cls, longitude, latitude):
        validate_point(longitude, latitude)
        geo = 'POINT({} {})'.format(longitude, latitude)
        station = Station(longitude=longitude, latitude=latitude, geo=geo)
